...
export default function StepBackground({
  ...
  return (
    <div>
      <p className="font-body text-sm font-medium text-[#0A1628] mb-2">Background</p>
      ...
      {value === 'solid' && (
        <div className="flex items-center gap-3">
          <label className="font-body text-xs text-[#0A1628]">Color</label>
          <input
            type="color"
            value={bgColor}
            onChange={(e) => onBgColorChange(e.target.value)}
            className="w-8 h-8 border-2 border-[#0A1628] rounded cursor-pointer"
          />
          <span className="font-body text-xs text-[#0A1628]/50">{bgColor}</span>
        </div>
      )}

      {value === 'gradient' && (
        <div className="flex items-center gap-3 flex-wrap">
          <label className="font-body text-xs text-[#0A1628]">From</label>
          <input
            type="color"
            value={bgGradient[0]}
            ...
            className="w-8 h-8 border-2 border-[#0A1628] rounded cursor-pointer"
          />
          <label className="font-body text-xs text-[#0A1628]">To</label>
          <input
            type="color"
            value={bgGradient[1]}
          ...
          />
          <div
            className="h-6 w-16 rounded border border-[#0A1628]/20"
